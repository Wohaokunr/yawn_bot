...
export function RelationsPanel({ groupId, readOnly = false }: { groupId: string; readOnly?: boolean }): React.JSX.Element {
  ...
  const view = searchParams.get("relations.view") === "graph" ? "graph" : "table";
  const typeFilter = searchParams.get("relations.type") ?? "";
  ...
  const setView = (value: string) => {
    ...
    if (value === "graph") next.set("relations.view", "graph"); else next.delete("relations.view");
    setSearchParams(next, { replace: true });
  };
  const setTypeFilter = (value: string) => {
    const next = new URLSearchParams(searchParams);
    if (value) next.set("relations.type", value); else next.delete("relations.type");
    setSearchParams(next, { replace: true });
    setPage(1);
  };
  ...
  const query = useApiQuery({
    ...
    invalidation: {
      resources: ["agent_relation", "agent_member_data", "agent_group_data"],
      scope: { groupId },
    },
  });
  const graphQuery = useApiQuery({
    queryKey: ["agent-relation-graph", groupId],
    fetcher: (signal) => api<AgentRelationGraph>(`/agent/groups/${groupId}/relations/graph`, { signal }).then((r) => r.data),
    invalidation: {
      resources: ["agent_relation", "agent_member_data", "agent_group_data"],
      scope: { groupId },
    },
  ...
  const typesQuery = useApiQuery({
    queryKey: ["agent-relation-types", groupId],
    fetcher: (signal) => api<string[]>(`/agent/groups/${groupId}/relations/types`, { signal }).then((r) => r.data),
    invalidation: { resources: ["agent_relation"], scope: { groupId } },
  });
  ...
  const remove = async (id: string) => {
    ...
    message.success("关系边已删除"); query.reload(); typesQuery.reload(); graphQuery.reload();
  };
  ...
  const openEdit = (row: AgentRelationItem) => {
    if (readOnly || query.stale) return;
    setEditing(row); editForm.setFieldsValue({ note: row.note, confidence: row.confidence });
  };
  ...
  return <>
    ...
    <Card title="成员关系边" extra={<Space wrap>
      ...
      <Select value={typeFilter} onChange={(value) => setTypeFilter(value)} style={{ width: 140 }} options={[{ value: "", label: "全部类型" }, ...typeOptions]} />
      {view === "table" && <Input.Search className="table-search" placeholder="搜索成员 QQ 号" allowClear onSearch={(v) => { setSearch(v); setPage(1); }} />}
      {!readOnly && <Button type="primary" disabled={query.stale} onClick={() => { setCreating(true); createForm.resetFields(); }}>新增关系边</Button>}
    </Space>}>{
      view === "graph"
        ? (graphQuery.error && !graph
          ? <QueryErrorAlert error={graphQuery.error} onRetry={graphQuery.reload} />
          : graph
            ? <Suspense fallback={<div className="rg-loading-wrap"><Spin /></div>}><LazyRelationGraphView graph={graph} typeFilter={typeFilter} readOnly={readOnly || graphQuery.stale} onEditRelation={readOnly || graphQuery.stale ? undefined : openEdit} onDeleteRelation={readOnly || graphQuery.stale ? undefined : (edge) => remove(edge.id)} /></Suspense>
        ...
        : (query.error && !query.data
          ...
          : <Table rowKey="id" loading={query.loading} dataSource={query.data?.rows ?? []} pagination={{ current: page, pageSize: 20, total: query.data?.total ?? 0, showSizeChanger: false, onChange: setPage }} locale={{ emptyText: <Empty description="暂无关系记忆" /> }} columns={readOnly ? [
            ...
            { title: "最后见到", dataIndex: "lastSeenAt", render: formatTime, width: 170 },
            { title: "操作", width: 120, render: (_, row: AgentRelationItem) => <Space><Button type="link" size="small" disabled={query.stale} onClick={() => openEdit(row)}>编辑</Button><Popconfirm title="删除这条关系边？" onConfirm={() => remove(row.id)}><Button type="link" size="small" danger disabled={query.stale}>删除</Button></Popconfirm></Space> },
          ]} />)
    }</Card>
