...
- 2026-08-14：修复番茄小说书籍搜索：改用公开搜索 API，支持书名/作者模糊搜索及相关、最新、最热排序；新增男/女频阅读榜和新书榜分类浏览，榜单书籍复用原有章节下载流程；补充空响应/验证码/结构异常的可读降级、榜单字体解码和无效菜单输入回归。未登录、未提交验证码、未绕过访问控制，未新增 ORM 迁移。
...
- 2026-08-14：番茄免费预览补全改为第三方 `GET /api/raw_full?item_id=...` 首选，默认地址来自开源 `mcp-server-fanqie` 文档，配置支持覆盖、超时和重试；只对阅读页同时明确 `needPay=0`、`isPaidPublication=false`、`isPaidStory=false` 的章节调用，并校验返回标题、全部免费段落和章节字数，失败后才回退自备 helper。目标 `https://fanqienovel.com/reader/7506075366054691352` 通过真实 provider 端到端：第 11 章标题匹配，返回正文 2148 字符，第三方元数据为 59/59 免费段落、章节字数 2021；番茄回归 18 项、Ruff、provider Pyright、compileall 和 diff check 通过。不得将第三方服务用于付费或元数据不明确章节，也不得发送 Cookie/账号凭据。
