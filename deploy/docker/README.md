...
The YawnBot application image no longer embeds Chromium. Fanqie browser search is provided by the optional `fanqie-browser` Playwright sidecar so users who do not need it avoid the browser download entirely.
...
Then build/start the optional Chromium headless sidecar:
...
The sidecar is reachable only through the Compose network; do not publish its Playwright server port to the Internet.
