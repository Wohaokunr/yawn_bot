...
def new_npc_fact_dict(fact_id: str) -> dict[str, Any]:
    """新建一条 NPC 私人情报。"""
    return {"id": fact_id, "name": "", "text": ""}


def new_social_strategy_dict() -> dict[str, Any]:
    """新建一条社交策略；可选覆写字段留给节点默认值。"""
    return {"skill": "persuade", "difficulty": "regular", "name": ""}


def new_social_node_dict(node_id: str) -> dict[str, Any]:
    """新建一个社交节点，并放入一条可立即编辑的默认策略。"""
    return {
        "id": node_id,
        "name": "",
        "goal": "",
        "strategies": [new_social_strategy_dict()],
    }
        ...
        for npc in get_list(data, "npcs"):
            ...
            for node in get_list(npc, "social_nodes"):
                if not isinstance(node, dict):
                    continue
                for ref_field in ("private_clues", "public_clues"):
                    clues = get_list(node, ref_field)
                    for i, clue_id in enumerate(clues):
                        if clue_id == old:
                            clues[i] = new
                            sites.append(
...
def rename_npc_fact(npc: dict[str, Any], old: str, new: str) -> list[str]:
    """重命名 NPC 私人情报，并更新该 NPC 社交节点的依赖列表。"""
    sites: list[str] = []
    for fact in get_list(npc, "facts"):
        if isinstance(fact, dict) and fact.get("id") == old:
            fact["id"] = new
            sites.append("NPC 情报自身 id")
            break
    for node in get_list(npc, "social_nodes"):
        if not isinstance(node, dict):
            continue
        for ref_field in ("requires_facts", "unlock_facts"):
            facts = get_list(node, ref_field)
            changed = False
            for i, fact_id in enumerate(facts):
                if fact_id == old:
                    facts[i] = new
                    changed = True
            if changed:
                sites.append(f"社交节点〈{node.get('id', '?')}〉{ref_field}")
    return sites
...
def clue_referrers(  # noqa: C901,PLR0912
    data: dict[str, Any], clue_id: str
) -> list[str]:
        ...
        for node in get_list(npc, "social_nodes"):
            ...
            for ref_field, label in (
                ...
                found.extend(
                    f"NPC〈{npc.get('id', '?')}〉社交节点"
                    f"〈{node.get('id', '?')}〉{label}"
                    for value in get_list(node, ref_field)
                    if value == clue_id
                )
