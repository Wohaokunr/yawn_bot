...
TOOL_RESULT_SPEECH_INSTRUCTION = (
    ...
    "看到 role=tool 后：先判断 ok；成功只提用户真正关心的结果，失败只说明可公开的失败原因和必要下一步。"
    "不要照抄 JSON、字段名、布尔值、内部 outcome/delivery_state、权限级别、trace、路径或协议细节。"
    "列表结果先概括数量，再按用户问题挑最相关项；除非用户明确要求，不要机械倾倒全部记录。"
    "写操作只有工具明确成功后才能用完成时；ok=false、超时或不确定状态不能说‘已经完成’。"
)
...
def tool_result_speech_hint(tool_name: str, payload: dict[str, Any]) -> str:
    ...
    if isinstance(result, dict):
        if isinstance(result.get("items"), list):
            return f"{name} 成功，返回 {len(result['items'])} 项；按用户问题筛选后自然表述。"
