...
- **命令层只校验、只投递**：所有输入必须经 `state.submit_action()` 入队；
  它执行队列容量、单用户配额、动作 id 去重与阶段快照检查。报名、退报名
  与房主移交也由引擎消费 `JOIN_GAME` / `LEAVE_GAME` 后修改，命令层不得直接改 `Game`。
...
- **多人节奏**：探索期每名存活调查员每轮仅一次主要行动，超时自动跳过；
...
| `request_team_check` | 当前场景的全部存活调查员参与；成功人数由系统钳制并逐人播报 |
