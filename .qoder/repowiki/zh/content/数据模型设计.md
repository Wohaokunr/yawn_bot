...
## 更新摘要
**所做更改**
- 新增三个权限相关的数据模型：GlobalUserFeature、GroupFeature、UserFeature
- 添加权限系统模块，实现功能开关的三级权限控制
- 更新实体关系图以包含新的权限模型
- 扩展依赖关系分析以反映权限系统的集成
...
**更新** 新增了完整的权限控制系统，支持全局用户级、群级别和用户级别的三级功能开关管理。

## 项目结构
yawn_core 插件采用 ORM 模型（基于 nonebot_plugin_orm / SQLAlchemy）组织数据模型，并通过 Alembic 进行数据库迁移。核心数据模型位于 data_models 目录下，包含 BotUser、BotGroup、UserGroup、CheckinRecord、CheckinUser、FriendRequest 六个基础实体；新增 GlobalUserFeature、GroupFeature、UserFeature 三个权限控制实体；业务逻辑在 checkin.py 与 friend_approve.py 中调用这些模型完成签到与好友申请审批流程，permission.py 模块提供权限检查功能。
...
章节来源
- [src/plugins/yawn_core/data_models/__init__.py:1-28](file://src/plugins/yawn_core/data_models/__init__.py#L1-L28)
...
- **GroupFeature**：群级别的功能开关，控制特定群组的功能可用性。
...
## 架构总览
下图展示数据模型之间的实体关系（ER 图），体现主键/外键与一对一/一对多关系，包含新增的权限控制模型。
...
BOTUSER ||--o{ GLOBALUSERFEATURE : "全局功能开关"
BOTGROUP ||--o{ GROUPFEATURE : "群组功能开关"
USERGROUP ||--o{ USERFEATURE : "用户功能覆盖"
```
...
- 关联关系
  ...
  - 与 GlobalUserFeature 一对多（一个用户可有多个功能开关）
- 使用场景
...
- 关联关系
  ...
  - 与 GroupFeature 一对多（一个群可有多个功能开关）
...
- 关联关系
  ...
  - 与 UserFeature 一对多（一个用户-群组合可有多个功能覆盖）
...
- 联合主键：(user_id, feature)
- 外键
  - user_id 引用 BotUser.user_id，级联删除
- 字段含义
  - feature：功能标识（字符串，最大64字符）
  - enabled：功能是否启用（布尔值，默认True）
  - updated_at：更新时间（自动维护）
- 使用场景
  - 控制用户在全局范围内的功能权限，适用于私聊和跨群功能

章节来源
- [src/plugins/yawn_core/data_models/global_user_feature.py:8-29](file://src/plugins/yawn_core/data_models/global_user_feature.py#L8-L29)

### GroupFeature（群组功能开关）
- 联合主键：(group_id, feature)
- 外键
  - group_id 引用 BotGroup.group_id，级联删除
- 字段含义
  - feature：功能标识（字符串，最大64字符）
  ...
  - updated_at：更新时间（自动维护）
- 使用场景
  - 控制特定群组的功能权限，支持群组级别的功能管理

章节来源
- [src/plugins/yawn_core/data_models/group_feature.py:8-29](file://src/plugins/yawn_core/data_models/group_feature.py#L8-L29)

### UserFeature（用户功能覆盖）
- 联合主键：(group_id, user_id, feature)
- 外键
  - (group_id, user_id) 引用 UserGroup(group_id, user_id)，级联删除
- 字段含义
  - feature：功能标识（字符串，最大64字符）
  - enabled：功能是否启用（布尔值，默认True）
  - updated_at：更新时间（自动维护）
- 使用场景
  - 在特定群组中覆盖用户的功能权限，优先级最高
...
BotUser "1" --> "many" GlobalUserFeature : "global_features"
BotGroup "1" --> "many" GroupFeature : "group_features"
UserGroup "1" --> "many" UserFeature : "user_features"
```
...
- 直接依赖
  ...
  - GlobalUserFeature 依赖 BotUser（通过外键）
  - GroupFeature 依赖 BotGroup（通过外键）
  - UserFeature 依赖 UserGroup（通过复合外键）
- 间接依赖
  - 签到流程依赖 BotUser、BotGroup、UserGroup、CheckinRecord、CheckinUser
  - 好友申请流程依赖 FriendRequest
  - 权限检查依赖 GlobalUserFeature、GroupFeature、UserFeature
- 潜在循环依赖
...
- 生命周期
  ...
  - **GlobalUserFeature**：用户功能开关随功能注册而创建，updated_at 自动维护
  - **GroupFeature**：群组功能开关随群组设置而创建，updated_at 自动维护
  - **UserFeature**：用户功能覆盖随权限配置而创建，updated_at 自动维护
- 保留策略
...
- 敏感字段
  ...
  - 功能开关配置涉及系统安全，需严格访问控制
- 访问控制
  - 好友申请审批命令（approve/reject/list）仅限超级用户执行
  - 权限检查通过 require_feature 装饰器实现细粒度控制
  - 建议在应用层增加角色校验与审计日志
- 数据最小化
  - 仅存储必要的用户与群信息，避免冗余
  - 权限开关仅存储必要的布尔状态和更新时间
...
- **权限检查失败**
  - 现象：用户无法使用已注册的功能
  - 排查：检查 FEATURE_REGISTRY 中是否包含该功能，确认权限解析链优先级
  - 处理：验证 GlobalUserFeature、GroupFeature、UserFeature 的配置状态
- **权限配置不生效**
  - 现象：修改权限后仍按旧配置执行
  - 排查：确认数据库连接正常，updated_at 字段是否正确更新
  - 处理：检查级联删除约束，确认外键关系正确
...
## 结论
YawnBot 的 yawn_core 插件通过清晰的数据模型与严格的约束，实现了用户-群关系管理与签到系统，以及好友申请审批流程。**新增的权限控制系统提供了三级功能开关管理（全局用户级、群级别、用户级别），支持灵活的功能权限控制**。模型设计兼顾了数据一致性与可扩展性，配合 Alembic 迁移可实现平滑演进。建议在后续迭代中完善索引策略、缓存机制与归档方案，以提升性能与可维护性。
...
- **权限系统迁移**
  - **待创建：需要为 GlobalUserFeature、GroupFeature、UserFeature 创建相应的迁移文件**
...
M4 --> M5["待创建<br/>创建权限相关表<br/>GlobalUserFeature/GroupFeature/UserFeature"]
M5 --> End(["结束"])
```
...
- **GlobalUserFeature**：user_id=123456，feature="checkin"，enabled=True，updated_at="2026-01-01T10:00:00"
- **GroupFeature**：group_id=789012，feature="info"，enabled=False，updated_at="2026-01-01T10:00:00"
- **UserFeature**：group_id=789012，user_id=123456，feature="checkin"，enabled=True，updated_at="2026-01-01T10:00:00"
...
participant DB as "数据库"
participant GUF as "GlobalUserFeature"
participant GF as "GroupFeature"
participant UF as "UserFeature"
U->>P : 尝试使用功能
P->>P : 检查是否为超级管理员
alt 是超级管理员
P-->>U : 允许访问
else 普通用户
P->>DB : 查询 UserFeature群内用户覆盖
alt 存在用户覆盖
P-->>U : 按用户覆盖状态决定
else 无用户覆盖
P->>DB : 查询 GroupFeature群级别开关
alt 存在群设置
P-->>U : 按群设置状态决定
else 无群设置
P->>DB : 查询 GlobalUserFeature全局设置
alt 存在全局设置
P-->>U : 按全局设置状态决定
else 无全局设置
P-->>U : 默认允许访问
