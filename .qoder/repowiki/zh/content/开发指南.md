...
> 更新说明（2026-08-13）：本文为历史生成文档，具体行号和模块列表可能滞后。
> 当前 `yawn_core` 同时包含权限、管理、提醒、AI 对话等平台能力，并动态加载
> `yawn_rpg`（YAML/schema/Textual 编辑器/AI KP）与 `yawn_werewolf`（状态机/AI
> 玩家）。共享模型客户端位于 `llm.py`，配置使用可选的 `AI_API_KEY`。
> canonical 迁移目录为 `data/nonebot_plugin_orm/migrations/`。开发命令、测试范围和
> 部署流程以源码、`README.md`、`.env.example`、`docs/deployment.md` 为准。
