...
## 项目结构
YawnBot 基于 NoneBot2 框架，采用插件化架构。核心配置文件为 pyproject.toml，依赖锁定由 uv.lock 提供。插件位于 src/plugins，当前包含 yawn_core 插件，提供签到等能力。数据模型与迁移脚本分别位于 data 与 src 下的 migrations 目录中。
...
- [uv.lock:1-541](file://uv.lock#L1-L541)
...
## 架构总览
YawnBot 以 NoneBot2 为核心，OneBot V11 作为消息适配器，ORM 插件负责持久化，apscheduler 驱动定时任务，sentry 负责异常上报，htmlkit 提供模板渲染。
...
subgraph "业务插件"
YC["yawn_core 插件<br/>签到/好友请求/信息/在线状态"]
end
subgraph "数据层"
...
## 结论
YawnBot 基于 NoneBot2 的插件化架构，配合 uv 的依赖锁定与 ORM 的数据管理能力，提供了稳定可扩展的机器人平台。通过合理的配置与部署策略，可在开发与生产环境中保持一致性与高性能表现。
