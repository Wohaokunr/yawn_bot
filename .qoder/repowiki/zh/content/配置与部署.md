...
- [src/plugins/yawn_core/ai_call.py](file://src/plugins/yawn_core/ai_call.py)
...
## 项目结构
YawnBot 基于 NoneBot2 框架，采用插件化架构。核心配置文件为 pyproject.toml，依赖锁定由 uv.lock 提供。插件位于 src/plugins，当前包含 yawn_core 插件，提供签到、AI 对话等能力。数据模型与迁移脚本分别位于 data 与 src 下的 migrations 目录中。
...
- [src/plugins/yawn_core/ai_call.py:1-57](file://src/plugins/yawn_core/ai_call.py#L1-L57)
...
- 注意：当前仓库未包含 .env 示例或环境变量清单，建议在部署时根据实际运行平台（如容器）注入必要的环境变量。
...
- [src/plugins/yawn_core/ai_call.py:4-9](file://src/plugins/yawn_core/ai_call.py#L4-L9)
...
- 迁移脚本：Alembic 迁移位于 data/nonebot_plugin_orm/migrations/yawn_core，用于创建/更新表结构与关系。
...
- 资源限制：在生产环境中限制进程数与内存上限，结合容器编排实现弹性伸缩。
...
- **新增**：OpenAI API 配置
  - 设置 OPENAI_API_KEY 环境变量存储 API 密钥
  - 配置 OPENAI_BASE_URL 环境变量指定 API 端点
  - 支持自定义模型名称和请求参数
...
- 环境变量
  ...
  - **新增**：配置 OPENAI_API_KEY 和 OPENAI_BASE_URL 环境变量
- 健康检查
  - 添加健康检查端点或命令，确保服务可用性
- **新增**：AI 服务优化
  - 配置 HTTP 连接池大小
