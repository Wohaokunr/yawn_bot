...
> 更新说明（2026-08-13）：本文为历史生成文档，后续章节可能仍含过时文件名、
> 行号或架构描述。当前项目以 `src/plugins/yawn_core` 为平台插件，包含
> `ai_chat.py`、`llm.py`、`reminder.py`、权限/管理模块，以及动态加载的
> `yawn_rpg` 与 `yawn_werewolf` 子插件。受版本控制的 canonical 迁移目录是
> `data/nonebot_plugin_orm/migrations/`（含根级迁移和 bind 子目录）；
> `src/**/migrations` 不是发布时的修改入口。配置、部署和运行边界以源码、根目录
> `README.md`、`.env.example` 与 `docs/deployment.md` 为准。
...
- 数据库 canonical 迁移存放于 `data/nonebot_plugin_orm/migrations/`；其中既有根级迁移，也有 `yawn_core/` bind 子目录。`src/**/migrations` 仅为历史副本，不应作为维护入口。
