...
## 简介
本文件为 YawnBot 的核心功能模块文档，聚焦以下四大能力：签到系统、好友申请管理、用户信息查询、用户行为追踪。文档从业务逻辑、数据模型、调用流程、接口设计、错误处理与性能优化等维度进行系统化说明，并提供可视化图示与常见问题解决方案，帮助初学者快速上手，同时为有经验的开发者提供深入的技术细节。

## 项目结构
YawnBot 基于 NoneBot2 插件体系，核心代码位于 src/plugins/yawn_core 下，包含四个主要功能模块与一组数据模型。插件通过 pyproject.toml 注册到 nonebot 的 plugin_dirs，并在运行时自动加载。
...
H->>DB : 更新 CheckinUser(累计/连续/积分)
H-->>U : 返回签到结果
end
Note over API : 必要时调用 get_group_info 补全群名
```

图表来源
- [presence.py:16-103](file://src/plugins/yawn_core/presence.py#L16-L103)
- [checkin.py:41-147](file://src/plugins/yawn_core/checkin.py#L41-L147)
- [friend_approve.py:31-64](file://src/plugins/yawn_core/friend_approve.py#L31-L64)
...
## 性能考量
- 签到唯一性约束在数据库层实现，避免应用层重复判断带来的竞争条件。
- presence 预处理对每条消息进行轻量读写，建议配合连接池与批量提交策略（当前逐条 commit，适合中小规模场景）。
- 群名获取为可选 API 调用，失败时降级记录空名称，避免阻塞主流程。
- 高频签到场景下，可考虑将 flush/commit 合并或使用事务批处理以降低 I/O 次数。
...
## 结论
YawnBot 核心模块围绕 NoneBot 事件机制与 ORM 数据层构建，具备清晰的职责划分与稳健的错误处理。签到系统通过数据库约束保障一致性；好友申请管理提供完整的审批闭环；用户信息查询聚合多维度数据；用户行为追踪实现自动化活跃度维护。建议在大规模部署时进一步优化事务批处理与 API 调用频率，以提升整体吞吐与稳定性。
